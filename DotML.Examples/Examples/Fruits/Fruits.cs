...
public class Fruits : BackpropExample
{
    ...
    public override Tensor<float> ParseUserInput(string input)
    {
        ...
        using var writer = new StreamWriter(Path.Combine(ProcessedDataPath, "input.scaled.xml"));
        tensor.SaveSpreadsheetML(writer);
        ...
        foreach (var file in files)
        {
            ...
            // Perform training augmentations
            {
                ...
                foreach (var augment in augments)
                {
                    ...
                    tWriter.Write((byte)classIndex);    // 0 - Classes.Length-1
            ...
            // Perform validation augmentations
            {
                ...
                foreach (var augment in augments)
                {
                    ...
                    vWriter.Write((byte)classIndex);    // 0 - Classes.Length-1
                    foreach (var val in tensor.AsSpan())
    ...
    public override void LoadTrainingData(out ITrainingDataSource<float> training, out ITrainingDataSource<float> validation)
    {
        ...
        using (var iReader = new BinaryReader(File.Open(Path.Combine(ProcessedDataPath, "training.bin"), FileMode.Open))) {
            while (iReader.BaseStream.Position < iReader.BaseStream.Length)
            {
                // Read output tensor
                var classIndex = iReader.ReadByte();
        ...
        using (var vReader = new BinaryReader(File.Open(Path.Combine(ProcessedDataPath, "validation.bin"), FileMode.Open))) {
            while (vReader.BaseStream.Position < vReader.BaseStream.Length)
            {
                // Read output tensor
                var classIndex = vReader.ReadByte();
                var output = oneHot[classIndex];
                ...
                for (var i = 0; i < span.Length; i++)
                {
                    span[i] = vReader.ReadByte() / 255.0f; // Scale from 0-255 to 0-1 scale
                }
    ...
    protected override bool HasTrainingProgressed(ModuleTrainingEnumerator.Report trainingReport)
    {
        // If accuracy if greater than previous save it
        // Want greater accuracy
        var accuracy = trainingReport.Metrics<AccuracyMetricsProvider>();
        if (!progress.HasValue || progress.Value < accuracy.Accuracy)
        {
    ...
    {
        if (output.Any((v) => float.IsNaN(v) || float.IsInfinity(v)))
            throw new ArgumentException(nameof(output), "Invalid numeric value in model output");
        //Console.WriteLine(string.Join(", ", output.AsArray()));
        //Console.WriteLine(string.Join(", ", Vec<float>.Wrap(output.AsArray()).SoftmaxNormalized()));
