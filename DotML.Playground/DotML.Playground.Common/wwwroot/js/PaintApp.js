...
BlazorComponents.PaintApp = (function(){
    ...
    var pixel_cache = null;
    var values_per_pixel = 4; // RGBA
    exports.cache_pixels = function(canvas, width, height) {
        const context = canvas.getContext('2d');
        var imaged = context.getImageData(0, 0, width, height).data;
        var array = new Uint8Array(width * height * 3);

        for (i=0, j=0, n=imaged.length; i < n; i+=values_per_pixel, j+=3) {
            // 1 pixel
            array[j]    = imaged[i];
            array[j+1]  = imaged[i+1];
            array[j+2]  = imaged[i+2];
    ...
    exports.get_chunk_of_cached = function(canvas, width, height, chunk_offset, chunk_size) {
        if (!pixel_cache)
            return new Uint8Array(0);

        var start_at = chunk_offset * 3;
        if (start_at > pixel_cache.length)
            return new Uint8Array(0);

        var end_at = start_at + chunk_size * 3;
        if (end_at > pixel_cache.length) {
            end_at = pixel_cache.length;
        }

        var result = new Uint8Array(end_at - start_at);
        for (var i = start_at, j = 0; i < end_at; i+=3, j+=3) {
            // 1 pixel
            result[j]    = pixel_cache[i];
            result[j+1]  = pixel_cache[i+1];
            result[j+2]  = pixel_cache[i+2];
            // i + 3 is alpha which is ignored
        }
